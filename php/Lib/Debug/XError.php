...
class XError
	...
	function Error($code, $msg, $file, $line, $obj){
        $this->show($code, $msg, $file, $line);
    }
    ...
    private function show($code, $msg, $file, $line, $type = 'Error'){

        $tmp = explode('||', $msg);
        $msg = isset($tmp[0]) ? trim($tmp[0]) : $msg;
        $helpCod = isset($tmp[1]) ? trim($tmp[1]) : 0;
        ...
        if($code == E_USER_ERROR
            && $msg == 'Page not found!') $img = $this->infoImg();
        else {
            $tmp = $this->errorType($code);
            $eType = $tmp['name'];
            $eText = $tmp['text'];
            $trace = $this->formatTrace($file, $line);
            $help  = $this->getHelp($helpCod);
            $debug = $this->debug();
            $img   = $this->errorImg();
        }

        //Pega uma página 404 do usuario em 'html/404.php';
        $p404 = _file_exists('html/404.php');
        include_once ($p404 !== false ? $p404 : __DIR__.'/page.php');

        exit();
    }


    private function getHelp($code){
        $h = _file_exists('help.phar');
        if($h !== false) {
            include_once 'phar://'.$h.'/main.php';
            return \Manual\Main::getErrorInfo($code);
        }
        return '';
    }
    ...
    function debug(){
        ...
        $MEM = Persist::this()->get('file_exists');
        if(!is_array($MEM)) return '';

        $key = array_keys($MEM[0]);
        $dbg = '<h3>Debug:</h3><table><tr><th>'.$key[0].'</th><th>'.$key[1].'</th></tr>';

        foreach (Persist::this()->get('file_exists') as $v) {
            $dbg .= '
            <tr>'.$this->debugList($v).'</tr>';
        }
        return $dbg.'
        </table>';
    }
    ...
    function debugList($arr){
        ...
        $dbg = '';
        foreach($arr as $k=>$v){
            $dbg .= '<td>'.$this->debugList($v).'</td>';
        }
        return $dbg;
    }

    private function formatTrace($file, $line){
        $trace = '<table>';
        $cont = 0;
    ...
    private function errorType($num){
        $manual = array(
    ...
    private function encodeImage ($filename=string,$filetype=string) {
        if ($filename) {
