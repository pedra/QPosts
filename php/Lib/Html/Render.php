...
class Render {
	...
	*/
	function _script($ret){ 
		...
		foreach ($link as $lk){
			$tag .= '<script type="text/javascript" src="'.$lk.'"></script>'."\n";
		}        
		return $tag;
	}
	...
	*/
	function _style($ret){
		...
		if(!is_array($link)) $link = array();
		foreach ($link as $lk){
			$tag .= '<link href="'.$lk.'"  rel="stylesheet" type="text/css"/>'."\n";
		}        
		return $tag;
	}
	...
	*/
	function _html($ret){ 
		if(isset($ret['file'])){
			$file = _file_exists('html/'.$ret['file'].'.html');
			if($file !== false){
				$doc = new Doc($ret['file']);
				return $doc->render()->getcontent();
			}
		} 
		return '';
	}
	...
	*/
	 ...
	 *
	 * Ex.: <x::module name="Article" action="getContent" class="aTitle" tag="h1" />
